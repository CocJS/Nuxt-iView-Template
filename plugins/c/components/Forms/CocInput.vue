<template>
  <div :class = "[computedStatusClasses.initHolder]">
    ...
    <Input
      ...
      @on-enter = "atomControllers.submit()">
    <slot 
      slot = "prepend" 
      name = "prepend"/>
    <slot 
    ...
    handleClasses(container, input, status) {
      Object.keys(this.computedStatusClasses).forEach(i => {
        if (!i.startsWith('init')) {
          let fun = status === i ? 'AddClass' : 'RemoveClass'
          container[fun](this.computedStatusClasses[i])
          input[fun](this.computedStatusClasses[i])
        } else {
          if (i.toLowerCase().includes('container')) {
            container.AddClass(this.computedStatusClasses[i])
          } else if (i.toLowerCase().includes('input')) {
            input.AddClass(this.computedStatusClasses[i])
          }
        }
      })
    }
